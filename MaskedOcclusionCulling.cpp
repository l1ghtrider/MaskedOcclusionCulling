...
//#error The MaskedOcclusionCulling.cpp should be compiled with lowest supported target platform
...
#define __mc float32x4_t
#define _mmc_loadu_ps vld1q_f32
#define _mmc_load1_ps(a) vdupq_n_f32((a)[0])
#define _mmc_add_ps vaddq_f32
#define _mmc_sub_ps vsubq_f32
#define _mmc_mul_ps vmulq_f32
#define _mmc_storeu_ps vst1q_f32
#else
#define __mc __m128
#define _mmc_loadu_ps _mm_loadu_ps
#define _mmc_load1_ps _mm_load1_ps
#define _mmc_add_ps _mm_add_ps
#define _mmc_sub_ps _mm_sub_ps
#define _mmc_mul_ps _mm_mul_ps
#define _mmc_storeu_ps _mm_storeu_ps
#endif
...
void MaskedOcclusionCulling::TransformVertices(const float* mtx, const float* inVtx, float* xfVtx, unsigned int nVtx, const VertexLayout& vtxLayout)
{
	...
	__mc mtxCol0 = _mmc_loadu_ps(mtx);
	__mc mtxCol1 = _mmc_loadu_ps(mtx + 4);
	__mc mtxCol2 = _mmc_loadu_ps(mtx + 8);
	__mc mtxCol3 = _mmc_loadu_ps(mtx + 12);
	...
	{
		__mc xVal = _mmc_load1_ps((float*)(vPtr));
		__mc yVal = _mmc_load1_ps((float*)(vPtr + vtxLayout.mOffsetY));
		__mc zVal = _mmc_load1_ps((float*)(vPtr + vtxLayout.mOffsetZ));

		__mc xform = _mmc_add_ps(_mmc_mul_ps(mtxCol0, xVal), _mmc_add_ps(_mmc_mul_ps(mtxCol1, yVal), _mmc_add_ps(_mmc_mul_ps(mtxCol2, zVal), mtxCol3)));
		_mmc_storeu_ps(outPtr, xform);
		vPtr += stride;
		outPtr += 4;
	}
}
...
MaskedOcclusionCulling* MaskedOcclusionCulling::Create(Implementation RequestedSIMD, pfnAlignedAlloc alignedAlloc, pfnAlignedFree alignedFree)
{
	...
	MaskedOcclusionCulling::Implementation impl = MaskedOcclusionCulling::NEON;

	if (object == nullptr)
		object = MaskedOcclusionCullingNeon::CreateMaskedOcclusionCulling(alignedAlloc, alignedFree);
...
{
#if defined(PLATFORM_WIN) || defined(PLATFORM_STANDALONE_WIN) || defined (__SSE4_1__)
